...
class CreateInviteRequest(BaseModel):
    ...
    code: str | None = Field(None, description="可选自定义邀请码")
    max_uses: int = Field(1, ge=1, le=100, description="最多可用次数")
    expires_in_days: int = Field(14, ge=1, le=365, description="有效天数")
    note: str = Field("", max_length=120, description="备注")
...
def load_sessions() -> Dict[str, Dict[str, Any]]:
    """读取服务端会话表，支持撤销 token。"""
    sessions = load_storage_json(SESSIONS_STORE_KEY, SESSIONS_PATH, {})
...
def save_sessions(sessions: Dict[str, Dict[str, Any]]) -> None:
    """保存服务端会话表。"""
    save_storage_json(SESSIONS_STORE_KEY, SESSIONS_PATH, sessions)


def load_invites() -> Dict[str, Dict[str, Any]]:
    """读取邀请码表；若为空则兼容 env 中的单一邀请码。"""
    invites = load_storage_json(INVITES_STORE_KEY, INVITES_PATH, {})
    if not isinstance(invites, dict):
        invites = {}
    if REGISTRATION_INVITE_CODE and REGISTRATION_INVITE_CODE not in invites:
        invites[REGISTRATION_INVITE_CODE] = {
            "code": REGISTRATION_INVITE_CODE,
            "status": "active",
            "max_uses": 9999,
            "used_count": 0,
            "note": "env 默认内测邀请码",
    ...
    return {str(key): value for key, value in invites.items() if isinstance(value, dict)}


def save_invites(invites: Dict[str, Dict[str, Any]]) -> None:
    """保存邀请码表。"""
    save_storage_json(INVITES_STORE_KEY, INVITES_PATH, invites)


def load_login_attempts() -> Dict[str, Dict[str, Any]]:
    """读取登录失败锁定状态。"""
    attempts = load_storage_json(LOGIN_ATTEMPTS_STORE_KEY, LOGIN_ATTEMPTS_PATH, {})
...
def build_platform_helper_commands(platform: str, helper_token: str, api_base_url: str) -> Dict[str, str]:
    ...
    windows = (
        ...
        f"& $py.Exe @pa $p --platform {quote_powershell(platform_key)} --api {quote_powershell(api_url)} --helper-token {quote_powershell(helper_token)}"
    )
    mac_linux = (
        "set -e; "
...
def parse_timestamp_epoch(value: str) -> int:
    """Parse local timestamp generated by current_timestamp; empty means never expires."""
    if not value:
        return 0
    try:
...
def future_timestamp(days: int) -> str:
    """Return a local timestamp days from now."""
    return time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time() + max(1, days) * 86400))


def login_attempt_key(login_id: str) -> str:
    """Stable key for login lockout tracking."""
    return hashlib.sha256(normalize_login_identifier(login_id).encode("utf-8")).hexdigest()[:24]


def get_login_lock(login_id: str) -> Dict[str, Any] | None:
    """Return current lockout state if the login identifier is locked."""
    attempts = load_login_attempts()
    record = attempts.get(login_attempt_key(login_id), {})
    locked_until = int(record.get("locked_until", 0) or 0)
    if locked_until > current_epoch():
        return {"locked_until": locked_until, "remaining_seconds": locked_until - current_epoch()}
    if record.get("locked_until"):
        record["locked_until"] = 0
        attempts[login_attempt_key(login_id)] = record
        save_login_attempts(attempts)
    return None


def record_login_failure(login_id: str) -> None:
    """Increment login failure count and lock after threshold."""
    attempts = load_login_attempts()
    key = login_attempt_key(login_id)
    record = attempts.get(key, {"failures": 0, "locked_until": 0})
    record["failures"] = int(record.get("failures", 0) or 0) + 1
    record["last_failed_at"] = current_timestamp()
    if record["failures"] >= LOGIN_FAILURE_LIMIT:
        record["locked_until"] = current_epoch() + LOGIN_LOCK_SECONDS
    attempts[key] = record
    save_login_attempts(attempts)


def clear_login_failures(login_id: str) -> None:
    """Clear login failure state after successful login."""
    attempts = load_login_attempts()
    key = login_attempt_key(login_id)
    if key in attempts:
        del attempts[key]
...
def validate_invite_code(code: str) -> Dict[str, Any]:
    """Validate invite status, usage and expiration."""
    normalized = str(code or "").strip()
    invites = load_invites()
    invite = invites.get(normalized)
    if not invite:
    ...
    expires_epoch = parse_timestamp_epoch(expires_at)
    if expires_epoch and expires_epoch < current_epoch():
    ...
    if used_count >= max_uses:
        raise HTTPException(status_code=403, detail="邀请码可用次数已耗尽。")
    return invite


def mark_invite_used(code: str, user_id: str) -> None:
    """Consume one invite usage."""
    invites = load_invites()
    invite = invites.get(str(code or "").strip())
    if not invite:
        return
    invite["used_count"] = int(invite.get("used_count", 0) or 0) + 1
    invite.setdefault("used_by", [])
    if isinstance(invite["used_by"], list):
        invite["used_by"].append({"user_id": user_id, "used_at": current_timestamp()})
...
def create_session_token(user: Dict[str, Any]) -> str:
    ...
    now = int(time.time())
    session_id = "ses_" + secrets.token_urlsafe(16).replace("-", "").replace("_", "")[:24]
    payload = {
    ...
    payload_part = b64url_encode(json.dumps(payload, ensure_ascii=False, separators=(",", ":")).encode("utf-8"))
    signature = hmac.new(OPERATOR_TOKEN.encode("utf-8"), payload_part.encode("ascii"), hashlib.sha256).digest()
    token = f"tk1.{payload_part}.{b64url_encode(signature)}"
    ...
    sessions[session_id] = {
        ...
        "role": payload["role"],
        "created_at": current_timestamp(),
        "expires_at": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(payload["exp"])),
        "revoked": False,
    }
...
def decode_session_payload(token: str) -> Dict[str, Any] | None:
    """Decode session token payload after signature verification."""
    try:
        prefix, payload_part, signature_part = token.split(".", 2)
        if prefix != "tk1":
            return None
        ...
        if not hmac.compare_digest(expected_signature, supplied_signature):
            return None
        return json.loads(b64url_decode(payload_part).decode("utf-8"))
    except Exception:
        return None


def revoke_session_token(token: str) -> bool:
    """Mark a session token as revoked server-side."""
    ...
    sessions[session_id]["revoked_at"] = current_timestamp()
    save_sessions(sessions)
    return True
...
async def login(payload: LoginRequest) -> Dict[str, Any]:
    ...
    lock = get_login_lock(login_id)
    if lock:
        raise HTTPException(status_code=423, detail=f"登录失败次数过多，请 {lock['remaining_seconds']} 秒后再试。")
    ...
    if not user_id or not user or user.get("status") != "active" or not verify_password(payload.password, str(user.get("password_hash") or "")):
        record_login_failure(login_id)
...
async def register(payload: RegisterRequest) -> Dict[str, Any]:
    ...
    invite = validate_invite_code(payload.invite_code)
    ...
    users[user_id] = user
    save_users(users)
    mark_invite_used(str(invite.get("code") or payload.invite_code).strip(), user_id)
    append_admin_audit("register", f"新用户注册：{username}。", {"user_id": user_id, "email": email, "invite_code": invite.get("code", "")})
...
async def auth_logout(request: Request) -> Dict[str, Any]:
    """撤销当前会话 token。"""
    token = extract_operator_token(request)
    revoked = revoke_session_token(token)
    append_admin_audit("logout", "用户主动登出。", {"revoked": revoked})
...
async def admin_list_users() -> Dict[str, Any]:
    ...
    for user_id, user in load_users().items():
        ...
        payload["products_count"] = len(ensure_products_shape(load_storage_json(f"{PRODUCTS_STORE_KEY}:user:{user_id}", DIAGNOSED_PRODUCTS_PATH, {})))
...
async def admin_update_user_status(user_id: str, payload: UserStatusRequest) -> Dict[str, Any]:
    """管理员启用或禁用用户。"""
    ...
    users[user_id]["status"] = status_value
    users[user_id]["updated_at"] = current_timestamp()
    save_users(users)
    if status_value == "disabled":
        ...
        save_sessions(sessions)
    append_admin_audit("update_user_status", f"用户状态更新：{user_id} -> {status_value}。")
    return {"status": "success", "user": public_user_payload(users[user_id])}


@app.get("/api/admin/invites")
async def admin_list_invites() -> Dict[str, Any]:
    """管理员查看邀请码列表。"""
    invites = list(load_invites().values())
    invites.sort(key=lambda item: item.get("created_at", ""), reverse=True)
    return {"status": "success", "invites": invites, "count": len(invites)}


@app.post("/api/admin/invites")
async def admin_create_invite(payload: CreateInviteRequest) -> Dict[str, Any]:
    """管理员创建邀请码。"""
    code = (payload.code or "tk-" + secrets.token_urlsafe(8).replace("-", "").replace("_", "")[:10]).strip()
    if len(code) < 6:
        raise HTTPException(status_code=400, detail="邀请码至少 6 位。")
    invites = load_invites()
    if code in invites:
    ...
    invite = {
        ...
        "created_at": current_timestamp(),
        "expires_at": future_timestamp(payload.expires_in_days),
        "created_by": actor.get("username", "admin"),
        "used_by": [],
    }
    invites[code] = invite
    save_invites(invites)
    append_admin_audit("create_invite", f"创建邀请码：{code}。", {"max_uses": payload.max_uses})
    return {"status": "success", "invite": invite}


@app.post("/api/admin/invites/{code}/disable")
async def admin_disable_invite(code: str) -> Dict[str, Any]:
    """管理员停用邀请码。"""
    invites = load_invites()
    if code not in invites:
        raise HTTPException(status_code=404, detail="邀请码不存在。")
    invites[code]["status"] = "disabled"
    invites[code]["updated_at"] = current_timestamp()
    save_invites(invites)
    append_admin_audit("disable_invite", f"停用邀请码：{code}。")
    return {"status": "success", "invite": invites[code]}
